...
Principle: 缺失值影响有效样本量，偏离值会拉动均值、相关、回归和方差分析。处理前先识别，处理后再复查。结构性缺失（整列在本文件中全为空，比如跨国数据里只属于其他国家的变量）不是"缺失数据"问题，是该变量不适用于本文件，不需要排除个案或插补，只需说明一次。

Automatic wording:

- Item-level missing: `频率/箱图显示 X 存在...个缺失/偏离值，缺失率为...%。若题目要求处理，可将非实质编码（如"不知道/拒答/无法选择"）设为用户缺失，或按题目指定方法替换缺失值。处理后需重新运行描述统计确认结果。`
- High missing rate (roughly >20-30%): `变量 X 的缺失率较高（...%），无论是剔除分析还是插补，结果的样本代表性都可能受限，解读时需注明这一限制。`
- Structural missing: `变量 X 在本数据文件中整列为空，属于该变量不适用于本组/本国，不计入缺失率，也不进行插补。`
- Exclusion vs imputation divergence: `剔除缺失值与插补后的结果在...（方向/显著性/效应量强度）上出现差异，以剔除结果为主要结论，插补结果仅作稳健性参考。`
