...
- Always pairs SPSS guidance as two explicit parts for every method: numbered menu-click steps (exact path, which variable goes in which box, every checkbox relevant to that question) plus a complete, directly runnable syntax block covering the same operations — never just one or the other.
...
- If given a raw `.sav`/`.csv`/`.xlsx` dataset: auto-detects missing-value codes from each variable's value labels (not a fixed code list), distinguishes structural missing (whole-column not-applicable, e.g. country-specific survey variables) from item-level missing, locates concept-named variables by searching column labels, audits recoding direction, runs the actual analysis (via installed IBM SPSS Statistics syntax when available, or `scripts/spss_exam_analyzer.py` as a Python fallback), sanity-checks the computed numbers for implausible values (negative adjusted R², |Beta| > 1, severe VIF, etc.) before reporting them, and — when the题目 doesn't specify a missing-value treatment — reports both the listwise/pairwise-exclusion result and an imputed-sensitivity result side by side as equally complete answers, flagging any divergence between them, instead of picking one silently.
