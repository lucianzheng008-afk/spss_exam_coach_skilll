...
0. `小题清单`（only when the题目 contains multiple sub-questions, e.g. numbered ①②③ / 1.2.3. / (一)(二)(三) / 多个问号句子）
   - Before writing the structured answer below, list every sub-question found, in the题目's own numbering, in one short line each. This is a coverage checklist, not the answer.
   - Every sub-question on this list must be answered later in the response — usually inside `结果自动描述与推论` or `考试作答模板`, one line/paragraph per sub-question, using the题目's own numbering so the user can match answer to question. Never silently merge two sub-questions into one combined sentence, and never skip a sub-question because it seems to repeat an earlier one (e.g. "哪个自变量解释度最高" and "哪个解释度最低" are two separate required answers even though they use the same Beta column).
   - If the题目 has multiple blocks (e.g. an ANOVA block and a regression block, or one block per country/group), repeat the full `方法判断` → `考试作答模板` structure once per block, then answer that block's sub-questions inside it. Do not interleave sub-questions from different blocks.
...
4. `SPSS操作步骤`
   - Split into two explicit, separately labeled sub-sections every time, even for a one-method question: `操作步骤` (menu clicks) and `语法` (a complete, directly runnable SPSS syntax block).
   - `操作步骤`: numbered steps, each step names the exact menu path (`分析 -> ...`), which variable goes into which named box, and every checkbox/button relevant to this specific question (not just the generic default set) — e.g. for regression: which boxes get 统计/估计/模型拟合/共线性诊断/德宾-沃森/个案诊断, and for the 图 sub-dialog which variable goes on which axis and which of 直方图/正态概率图 are checked. Include preprocessing steps first (missing-value handling, recoding) as their own numbered steps, not folded into the main procedure step.
   - `语法`: one fenced ```spss code block containing every command needed to reproduce the operation steps end to end — `GET FILE`, any `MISSING VALUES`/`RECODE`/`COMPUTE` preprocessing, the analysis command with every sub-option used in the steps above (e.g. full `/STATISTICS`, `/CRITERIA`, `/METHOD`, `/SCATTERPLOT`, `/RESIDUALS`, `/SAVE` clauses for `REGRESSION`; full `/STATISTICS`, `/POSTHOC`, `/PLOT` for ONEWAY/UNIANOVA; full `/MISSING`, `/PRINT` for CORRELATIONS/NONPAR CORR). Do not give a shortened or partial syntax that omits an option mentioned in the steps — steps and syntax must cover exactly the same operations. Use Mac-style file paths (`/Users/...`) when the user's environment is macOS.
   - If the题目 needs more than one method (e.g. recode then regression, or regression then post-hoc), give one `操作步骤`+`语法` pair per method, in execution order, not a single merged block.
   - Immediately after every `语法` code block, add one short line reminding the user to copy only the code fenced inside that block into the SPSS syntax window — not the surrounding prose, headings, or table markdown — since pasting markdown-formatted text into SPSS syntax produces a `无法将行中的第一个单词识别为 SPSS Statistics 命令` error on every non-syntax line. Keep this reminder to one line; do not repeat the full explanation of why the error happens unless the user actually hits it.
...
6. `结果自动描述与推论`
   ...
   - Whenever the题目 asks 为什么/请说明理由, or asks a comparative/superlative question (哪个最高/最低/是否满足/哪两组显著), follow the three-layer "为什么" depth required in `references/result-interpretation-principles.md` (结论 → 判断依据的具体统计量 → 该统计量为何是正确依据的原理性理由, aligned with the textbook/课件 principle when one exists in `textbook-ocr-notes.md`/`textbook-integration.md`). A bare number restatement without the principle is not sufficient.
...
15. Default to proceeding and flagging, not stopping to ask, when a contradiction is internally resolvable from the data itself:
    - Examples that should be resolved by proceeding-with-a-flag: a file name implies one country/group but the `country`/group variable inside the data says another; a variable's value labels contradict the recoding direction the题目 assumes; a requested grouping variable is entirely missing in one file but a clearly equivalent substitute exists in the same file (state the substitution and why).
    - In these cases: run the analysis on what the data actually contains, state the contradiction explicitly and prominently (e.g. in `数据核验`, in bold, near the top of that block's answer), and let the user decide afterward whether to redo it with corrected data — do not block the entire answer on a clarifying question when computation can still proceed meaningfully.
    - Only stop and ask before computing when the ambiguity is not resolvable from the data itself — e.g. two equally plausible candidate columns for the same concept with no way to disambiguate from labels, a missing variable with no plausible substitute, or a request whose intended method genuinely depends on information only the user has (e.g. whether a categorical variable should be treated as ordinal or nominal for a borderline case, when the题目 doesn't say).
    - When in doubt between the two, prefer proceeding-with-a-flag: a flagged answer the user can correct costs less of their time than a blocked turn waiting on a clarification that turns out to be unnecessary.
